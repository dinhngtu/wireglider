...
void radix_tree_node_rcu_free(struct rcu_head *head) {
    struct xa_node *node = caa_container_of(head, struct xa_node, rcu_head);
...
bool xas_nomem(struct xa_state *xas, gfp_t gfp) {
    ...
    xas->xa_alloc = static_cast<xa_node *>(calloc(1, sizeof(xa_node)));
    if (!xas->xa_alloc)
        return false;
...
static void *xas_alloc(struct xa_state *xas, unsigned int shift) {
    ...
    if (node) {
        ...
        node = static_cast<xa_node *>(calloc(1, sizeof(xa_node)));
        if (!node) {
...
static int xas_expand(struct xa_state *xas, void *head) {
    ...
    while (max > max_index(head)) {
        ...
        XA_NODE_BUG_ON(node, shift > BITS_PER_LONG);
        node = static_cast<xa_node *>(xas_alloc(xas, shift));
        if (!node)
            return -ENOMEM;
...
#ifdef CONFIG_XARRAY_MULTI
static void xas_set_range(struct xa_state *xas, unsigned long first, unsigned long last) {
    unsigned int shift = 0;
 ...
 */
int __xa_alloc(struct xarray *xa, uint32_t *id, void *entry, struct xa_limit limit, gfp_t gfp) {
    XA_STATE(xas, xa, 0);
 ...
 */
int __xa_alloc_cyclic(struct xarray *xa, uint32_t *id, void *entry, struct xa_limit limit, uint32_t *next, gfp_t gfp) {
    uint32_t min = limit.min;
    int ret;

    limit.min = std::max(min, *next);
...
static bool xas_sibling(struct xa_state *xas) {
    ...
    if (!/*IS_ENABLED(CONFIG_XARRAY_MULTI)*/ false || !node)
        return false;
    mask = (XA_CHUNK_SIZE << node->shift) - 1;
    return (xas->xa_index & mask) > ((unsigned long)xas->xa_offset << node->shift);
}
...
static unsigned int xas_extract_present(struct xa_state *xas, void **dst, unsigned long max, unsigned int n) {
    void *entry;
    unsigned int i = 0;
...
static unsigned int xas_extract_marked(
    struct xa_state *xas,
    void **dst,
    unsigned long max,
    unsigned int n,
...
void xa_dump_node(const struct xa_node *node) {
    ...
    pr_cont(
        "node %px %s %d parent %px shift %d count %d values %d "
        "array %px list %px %px marks",
        node,
        node->parent ? "offset" : "max",
        node->offset,
        node->parent,
        node->shift,
        node->count,
...
void xa_dump_entry(const void *entry, unsigned long index, unsigned long shift) {
    ...
    if (xa_is_node(entry)) {
        if (shift == 0) {
            ...
            xa_dump_node(node);
            for (i = 0; i < XA_CHUNK_SIZE; i++)
                xa_dump_entry(node->slots[i], index + (i << node->shift), node->shift);
        }
...
void xa_dump(const struct xarray *xa) {
    ...
    pr_info(
        "xarray: %px head %px flags %x marks %d %d %d\n",
        xa,
        entry,
        xa->xa_flags,
