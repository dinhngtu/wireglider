...
static __always_inline enum maple_type mte_node_type(const struct maple_enode *entry) {
    return (maple_type)(((unsigned long)entry >> MAPLE_NODE_TYPE_SHIFT) & MAPLE_NODE_TYPE_MASK);
}
...
static __always_inline struct maple_node *mte_to_node(const struct maple_enode *entry) {
...
static inline maple_enode *mte_mk_root(const struct maple_enode *node) {
    return (maple_enode *)((unsigned long)node | MAPLE_ROOT_NODE);
}

static inline maple_enode *mte_safe_root(const struct maple_enode *node) {
    return (maple_enode *)((unsigned long)node & ~MAPLE_ROOT_NODE);
}
 ...
 */
static __always_inline unsigned int mte_parent_slot(const struct maple_enode *enode) {
 ...
 */
static __always_inline struct maple_node *mte_parent(const struct maple_enode *enode) {
 ...
 * otherwise.
 */
static __always_inline unsigned long mas_safe_pivot(
...
static __always_inline maple_enode *mt_slot(const struct maple_tree *mt, void __rcu **slots, unsigned char offset) {
    return (maple_enode *)rcu_dereference_check(slots[offset], mt_locked(mt));
}

static __always_inline maple_enode *mt_slot_locked(struct maple_tree *mt, void __rcu **slots, unsigned char offset) {
    return (maple_enode *)rcu_dereference_protected(slots[offset], mt_write_locked(mt));
}
/*
 ...
 */
static __always_inline maple_enode *mas_slot_locked(struct ma_state *mas, void __rcu **slots, unsigned char offset) {
 ...
 * Return: The entry stored in @slots at the @offset
 */
static __always_inline maple_enode *mas_slot(struct ma_state *mas, void __rcu **slots, unsigned char offset) {
 ...
 */
static __always_inline maple_enode *mas_root(struct ma_state *mas) {
    return (maple_enode *)rcu_dereference_check(mas->tree->ma_root, mt_locked(mas->tree));
}

static inline maple_enode *mt_root_locked(struct maple_tree *mt) {
    return (maple_enode *)rcu_dereference_protected(mt->ma_root, mt_write_locked(mt));
}
 ...
 * Return: The pointer to the root of the tree
 */
static inline maple_enode *mas_root_locked(struct ma_state *mas) {
 ...
 */
static __always_inline unsigned char ma_data_end(
    struct maple_node *node,
    enum maple_type type,
    unsigned long *pivots,
    unsigned long max) {
 ...
 */
static inline void mas_parent_gap(struct ma_state *mas, unsigned char offset, unsigned long new) {
 ...
 * @end: The node end
 */
static inline void mas_leaf_set_meta(struct maple_node *node, enum maple_type mt, unsigned char end) {
...
static noinline_for_kasan void mas_store_b_node(
    ...
    if (piv + 1 < mas->index) {
        ...
        b_node->slot[b_end] = (maple_enode *)wr_mas->content;
        if (!wr_mas->content)
    ...
    /* Store the new entry. */
    mas->offset = b_end;
    b_node->slot[b_end] = (maple_enode *)wr_mas->entry;
 ...
 */
static inline void mas_node_or_none(struct ma_state *mas, struct maple_enode *enode) {
    if (enode) {
        mas->node = enode;
        mas->status = ma_active;
    } else {
...
static inline void mab_set_b_end(struct maple_big_node *b_node, struct ma_state *mas, void *entry) {
    ...
    b_node->slot[b_node->b_end] = (maple_enode *)entry;
    if (mt_is_alloc(mas->tree))
 ...
 */
static inline void mas_topiary_node(struct ma_state *mas, struct ma_state *tmp_mas, bool in_rcu) {
 ...
 */
static inline void mas_split_final_node(struct maple_subtree_state *mast, struct ma_state *mas, int height) {
...
static inline void mas_wr_walk_traverse(struct ma_wr_state *wr_mas) {
    ...
    wr_mas->mas->node = (maple_enode *)wr_mas->content;
    wr_mas->mas->offset = 0;
    wr_mas->mas->depth++;
}
/*
...
static inline void mas_extend_spanning_null(struct ma_wr_state *l_wr_mas, struct ma_wr_state *r_wr_mas) {
    ...
    if (!r_wr_mas->content) {
        ...
        r_mas->last = mas_safe_pivot(r_mas, r_wr_mas->pivots, r_wr_mas->type, (maple_type)(r_mas->offset + 1));
        r_mas->offset++;
    }
}
...
static inline void mas_wr_end_piv(struct ma_wr_state *wr_mas) {
    while ((wr_mas->offset_end < wr_mas->mas->end) && (wr_mas->mas->last > wr_mas->pivots[wr_mas->offset_end]))
        wr_mas->offset_end++;
...
static __always_inline bool mas_rewalk_if_dead(
    struct ma_state *mas,
    struct maple_node *node,
 ...
 * Return: 1 on dead node, 0 otherwise.
 */
static int mas_next_node(struct ma_state *mas, struct maple_node *node, unsigned long max) {
    unsigned long min;
    unsigned long *pivots;
    struct maple_enode *enode;
    struct maple_node *tmp;
...
static inline bool mas_anode_descend(struct ma_state *mas, unsigned long size) {
    enum maple_type type = mte_node_type(mas->node);
    unsigned long pivot, _min, gap = 0;
    ...
    gaps = ma_gaps(node, type);
    offset = mas->offset;
    _min = mas_safe_min(mas, pivots, offset);
    ...
    for (; offset <= data_end; offset++) {
        ...
        if (gaps)
            ...
            gap = min(pivot, mas->last) - max(mas->index, _min) + 1;
        else
            goto next_slot;
...
static void __rcu **mte_dead_walk(struct maple_enode **enode, unsigned char offset) {
    ...
    do {
        *enode = ma_enode_ptr(next);
        ...
        next = (maple_node *)rcu_dereference_protected(slots[offset], lock_is_held(&rcu_callback_map));
        offset = 0;
...
static void mt_free_walk(struct rcu_head *head) {
    ...
    do {
        ...
        if ((offset < mt_slots[type]) && rcu_dereference_protected(slots[offset], lock_is_held(&rcu_callback_map)))
 ...
 * Returns: True if entry is the answer, false otherwise.
 */
static bool mas_find_rev_setup(struct ma_state *mas, unsigned long min, void **entry) {
 ...
 */
static inline void mas_copy_node(struct ma_state *mas, struct ma_state *new_mas, struct maple_pnode *parent) {
    struct maple_node *node = mte_to_node(mas->node);
 ...
 */
static inline void mas_dup_alloc(struct ma_state *mas, struct ma_state *new_mas, gfp_t gfp) {
    struct maple_node *node = mte_to_node(mas->node);
    struct maple_node *new_node = mte_to_node(new_mas->node);
    enum maple_type type;
...
static inline void mas_dup_build(struct ma_state *mas, struct ma_state *new_mas, gfp_t gfp) {
    ...
    if (unlikely(mt_attr(mas->tree) != mt_attr(new_mas->tree)) || unlikely(!mtree_empty(new_mas->tree))) {
        mas_set_err(mas, -EINVAL);
        return;
    }
...
void __mt_destroy(struct maple_tree *mt) {
    maple_enode *root = mt_root_locked(mt);
