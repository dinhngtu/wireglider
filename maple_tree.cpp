...
static inline struct maple_node *mt_alloc_one(gfp_t gfp) {
    ...
    return static_cast<maple_node *>(aligned_alloc(sizeof(maple_node), sizeof(maple_node)));
}

static inline int mt_alloc_bulk(gfp_t gfp, size_t size, void **nodes) {
    // return kmem_cache_alloc_bulk(maple_node_cache, gfp, size, nodes);
    for (size_t i = 0; i < size; i++)
        nodes[i] = aligned_alloc(sizeof(maple_node), sizeof(maple_node));
    return size;
}
...
static void mt_free_rcu(struct rcu_head *head) {
    struct maple_node *node = caa_container_of(head, struct maple_node, rcu);
...
static inline enum maple_type mte_node_type(const struct maple_enode *entry) {
    return static_cast<maple_type>(((unsigned long)entry >> MAPLE_NODE_TYPE_SHIFT) & MAPLE_NODE_TYPE_MASK);
}
...
static inline struct maple_enode *mt_mk_node(const struct maple_node *node, enum maple_type type) {
    return (maple_enode *)((unsigned long)node | (type << MAPLE_ENODE_TYPE_SHIFT) | MAPLE_ENODE_NULL);
}
 ...
 */
static inline enum maple_type mas_parent_type(struct ma_state *mas, struct maple_enode *enode) {
    unsigned long p_type;

    p_type = (unsigned long)mte_to_node(enode)->parent;
    if (WARN_ON(p_type & MAPLE_PARENT_ROOT))
        return static_cast<maple_type>(0);
 ...
 * parent type.
 */
static inline void mas_set_parent(
    struct ma_state *mas,
...
static inline struct maple_node *mte_parent(const struct maple_enode *enode) {
    return (maple_node *)((unsigned long)(mte_to_node(enode)->parent) & ~MAPLE_NODE_MASK);
}
...
static inline bool ma_dead_node(const struct maple_node *node) {
    ...
    smp_rmb();
    parent = (maple_node *)((unsigned long)node->parent & ~MAPLE_NODE_MASK);
    return (parent == node);
}
 ...
 */
static inline unsigned long *ma_gaps(struct maple_node *node, enum maple_type type) {
    switch (type) {
    case maple_arange_64:
 ...
 * otherwise.
 */
static inline unsigned long mas_safe_pivot(
    const struct ma_state *mas,
 ...
 */
static inline void mte_set_pivot(struct maple_enode *mn, unsigned char piv, unsigned long val) {
...
static inline void *mt_slot_locked(struct maple_tree *mt, void __rcu **slots, unsigned char offset) {
    return rcu_dereference(slots[offset]);
}
/*
 ...
 */
static inline void *mas_slot_locked(struct ma_state *mas, void __rcu **slots, unsigned char offset) {
 ...
 * Return: The entry stored in @slots at the @offset
 */
static inline void *mas_slot(struct ma_state *mas, void __rcu **slots, unsigned char offset) {
 ...
 * @end: The end of the data in this node.
 */
static inline void mt_clear_meta(struct maple_tree *mt, struct maple_node *mn, enum maple_type type) {
    ...
    case maple_range_64:
        ...
        if (unlikely(pivots[MAPLE_RANGE64_SLOTS - 2])) {
            slots = mn->mr64.slot;
            next = static_cast<maple_enode *>(mt_slot_locked(mt, slots, MAPLE_RANGE64_SLOTS - 1));
 ...
 * @mt: The maple node type
 */
static inline unsigned char ma_meta_gap(struct maple_node *mn, enum maple_type mt) {
 ...
 */
static inline void ma_set_meta_gap(struct maple_node *mn, enum maple_type mt, unsigned char offset) {
 ...
 */
static inline void mat_add(struct ma_topiary *mat, struct maple_enode *dead_enode) {
...
static void mt_destroy_walk(struct maple_enode *enode, struct maple_tree *mt, bool free);
/*
...
static inline void mas_descend(struct ma_state *mas) {
    ...
    mas->node = static_cast<maple_enode *>(mas_slot(mas, slots, mas->offset));
}
 ...
 * @val: The gap value
 */
static inline void mte_set_gap(const struct maple_enode *mn, unsigned char gap, unsigned long val) {
...
static inline void mas_alloc_nodes(struct ma_state *mas, gfp_t gfp) {
    ...
    while (requested) {
        ...
        max_req = std::min(requested, max_req);
...
static void mas_node_count(struct ma_state *mas, int count) {
    return mas_node_count_gfp(mas, count, /*GFP_NOWAIT | __GFP_NOWARN*/ 0);
}
...
static inline struct maple_enode *mas_start(struct ma_state *mas) {
    ...
    retry:
        mas->depth = 0;
        root = static_cast<maple_enode *>(mas_root(mas));
        /* Tree with nodes */
        if (likely(xa_is_node(root))) {
            mas->depth = 1;
            mas->node = static_cast<maple_enode *>(mte_safe_root(root));
            mas->offset = 0;
 ...
 */
static inline unsigned char ma_data_end(
    struct maple_node *node,
    enum maple_type type,
    unsigned long *pivots,
    unsigned long max) {
 ...
 */
static inline unsigned long ma_max_gap(
    struct maple_node *node,
    unsigned long *gaps,
    enum maple_type mt,
    unsigned char *off) {
 ...
 */
static inline void mas_parent_gap(struct ma_state *mas, unsigned char offset, unsigned long newgap) {
...
ascend:
    ...
    } else if (newgap < meta_gap) {
        newgap = ma_max_gap(pnode, pgaps, pmt, &meta_offset);
 ...
 */
static inline void mas_adopt_children(struct ma_state *mas, struct maple_enode *parent) {
    ...
    do {
        child = static_cast<maple_enode *>(mas_slot_locked(mas, slots, offset));
 ...
 */
static inline void mas_put_in_tree(struct ma_state *mas, struct maple_enode *old_enode)
 ...
 */
static inline void mas_replace_node(struct ma_state *mas, struct maple_enode *old_enode)
    __must_hold(mas->tree->ma_lock) {
 ...
 */
static inline bool mas_find_child(struct ma_state *mas, struct ma_state *child) __must_hold(mas->tree->ma_lock) {
    enum maple_type mt;
    unsigned char offset;
    unsigned char end;
    unsigned long *pivots;
    ...
    for (offset = mas->offset; offset <= end; offset++) {
        entry = static_cast<maple_enode *>(mas_slot_locked(mas, slots, offset));
 ...
 */
static inline void mab_shift_right(struct maple_big_node *b_node, unsigned char shift) {
 ...
 */
static inline bool mab_middle_node(struct maple_big_node *b_node, int split, unsigned char slot_count) {
 ...
 */
static inline int mab_no_null_split(struct maple_big_node *b_node, unsigned char split, unsigned char slot_count) {
    if (!b_node->slot[split]) {
        /*
         * If the split is less than the max slot && the right side will
         * still be sufficient, then increment the split on NULL.
         */
        if ((split < slot_count - 1) && (b_node->b_end - split) > (mt_min_slots[b_node->type]))
            split++;
        else
            split--;
    }
 ...
 */
static inline int mab_calc_split(
    struct ma_state *mas,
    struct maple_big_node *bn,
    ...
    if (unlikely(mab_middle_node(bn, split, slot_count))) {
        ...
        *mid_split = 0;
         ...
         */
        while ((split < slot_count - 1) && ((bn->pivot[split] - min) < slot_count - 1) && (b_end - split > slot_min))
            split++;
    }
...
static inline void mas_mab_cp(
    ...
    piv_end = std::min(mas_end, mt_pivots[mt]);
    for (; i < piv_end; i++, j++) {
        b_node->pivot[j] = pivots[i];
...
complete:
    ...
    if (!ma_is_leaf(mt) && mt_is_alloc(mas->tree)) {
        gaps = ma_gaps(node, mt);
        memcpy(b_node->gap + mab_start, gaps + mas_start, sizeof(unsigned long) * j);
    }
}
 ...
 * node during a write.
 */
static inline void mas_leaf_set_meta(
    struct ma_state *mas,
    struct maple_node *node,
    unsigned long *pivots,
    enum maple_type mt,
 ...
 * @mt: The maple node type
 */
static inline void mas_bulk_rebalance(struct ma_state *mas, unsigned char end, enum maple_type mt) {
 ...
 */
static noinline_for_kasan void mas_store_b_node(
    ...
    if (piv + 1 < mas->index) {
        ...
        b_node->slot[b_end] = static_cast<maple_enode *>(wr_mas->content);
        if (!wr_mas->content)
    ...
    /* Store the new entry. */
    mas->offset = b_end;
    b_node->slot[b_end] = static_cast<maple_enode *>(wr_mas->entry);
...
static inline void mas_wr_node_walk(struct ma_wr_state *wr_mas) {
    ...
    count = wr_mas->node_end = ma_data_end(wr_mas->node, wr_mas->type, wr_mas->pivots, mas->max);
    offset = mas->offset;
...
static inline void mast_rebalance_next(struct maple_subtree_state *mast) {
    ...
    mas_mab_cp(mast->orig_r, 0, mt_slot_count(mast->orig_r->node), mast->bn, b_end);
 ...
 */
static inline struct maple_enode *mas_new_ma_node(struct ma_state *mas, struct maple_big_node *b_node) {
 ...
 */
static inline unsigned char mas_mab_to_node(
    struct ma_state *mas,
    struct maple_big_node *b_node,
 ...
 */
static inline void mab_set_b_end(struct maple_big_node *b_node, struct ma_state *mas, void *entry) {
    if (!entry)
        return;

    b_node->slot[b_node->b_end] = static_cast<maple_enode *>(entry);
    if (mt_is_alloc(mas->tree))
 ...
 */
static inline void mte_mid_split_check(
    struct maple_enode **l,
 ...
 * @split - the split location.
 */
static inline void mast_set_split_parents(
 ...
 */
static inline void mas_topiary_node(struct ma_state *mas, struct maple_enode *enode, bool in_rcu) {
 ...
 *
 */
static inline void mas_topiary_replace(struct ma_state *mas, struct maple_enode *old_enode) {
    ...
    do {
        ...
        for (i = 0; i < 3; i++) {
            ...
            while (n < 3) {
                ...
                if ((tmp_next[n].min >= tmp_next->index) && (tmp_next[n].max <= tmp_next->last)) {
 ...
 */
static inline void mas_wmb_replace(struct ma_state *mas, struct maple_enode *old_enode) {
 ...
 */
static inline void mast_cp_to_nodes(
    struct maple_subtree_state *mast,
    struct maple_enode *left,
    struct maple_enode *middle,
    struct maple_enode *right,
...
static inline void mast_combine_cp_right(struct maple_subtree_state *mast) {
    ...
    mas_mab_cp(mast->orig_r, mast->orig_r->offset + 1, mt_slot_count(mast->orig_r->node), mast->bn, mast->bn->b_end);
    mast->orig_r->last = mast->orig_r->max;
}
...
static inline void *mtree_range_walk(struct ma_state *mas) {
    ...
    next:
        slots = ma_slots(node, type);
        next = static_cast<maple_enode *>(mt_slot(mas->tree, slots, offset));
        if (unlikely(ma_dead_node(node)))
            goto dead_node;
 ...
 */
static int mas_spanning_rebalance(struct ma_state *mas, struct maple_subtree_state *mast, unsigned char count) {
    ...
    while (count--) {
        ...
        split = mas_mab_to_node(mas, mast->bn, &left, &right, &middle, &mid_split, mast->orig_l->min);
    ...
    l_mas.node = mt_mk_node(ma_mnode_ptr(mas_pop_node(mas)), mte_node_type(mast->orig_l->node));
    l_mas.depth++;
...
static inline void mas_destroy_rebalance(struct ma_state *mas, unsigned char end) {
    ...
    mas->min = l_mas.max + 1;
    old_eparent = mt_mk_node(mte_parent(l_mas.node), mas_parent_type(&l_mas, l_mas.node));
    tmp += end;
    if (!in_rcu) {
        ...
        mte_set_pivot(old_eparent, mte_parent_slot(l_mas.node), l_pivs[split]);
 ...
 */
static inline bool mas_split_final_node(struct maple_subtree_state *mast, struct ma_state *mas, int height) {
 ...
 */
static inline void mast_fill_bnode(struct maple_subtree_state *mast, struct ma_state *mas, unsigned char skip) {
    bool cp = true;
    unsigned char split;
    ...
    if (cp)
        mas_mab_cp(mas, split + skip, mt_slot_count(mas->node) - 1, mast->bn, mast->bn->b_end);
 ...
 */
static inline void mast_split_data(struct maple_subtree_state *mast, struct ma_state *mas, unsigned char split) {
    ...
    mas_set_split_parent(mast->orig_l, mast->l->node, mast->r->node, &p_slot, split);
    mas_set_split_parent(mast->orig_r, mast->l->node, mast->r->node, &p_slot, split);
}
 ...
 */
static inline bool mas_push_data(struct ma_state *mas, int height, struct maple_subtree_state *mast, bool left) {
 ...
 */
static inline bool mas_reuse_node(struct ma_wr_state *wr_mas, struct maple_big_node *bn, unsigned char end) {
    /* Need to be rcu safe. */
 ...
 * @end: The end of the data.
 */
static noinline_for_kasan int mas_commit_b_node(
    struct ma_wr_state *wr_mas,
    struct maple_big_node *b_node,
    unsigned char end) {
    struct maple_node *node;
    ...
    if ((b_end < mt_min_slots[b_type]) && (!mte_is_root(old_enode)) && (mas_mt_height(wr_mas->mas) > 1))
...
static inline void mas_wr_walk_traverse(struct ma_wr_state *wr_mas) {
    ...
    wr_mas->mas->node = static_cast<maple_enode *>(wr_mas->content);
    wr_mas->mas->offset = 0;
    wr_mas->mas->depth++;
}
/*
 ...
 */
static inline void mas_extend_spanning_null(struct ma_wr_state *l_wr_mas, struct ma_wr_state *r_wr_mas) {
    ...
    if ((l_mas->index == l_wr_mas->r_min) && (l_slot && !mas_slot_locked(l_mas, l_wr_mas->slots, l_slot - 1))) {
        if (l_slot > 1)
    ...
    if (!r_wr_mas->content) {
        ...
        (r_mas->last == r_wr_mas->r_max) && (r_mas->last < r_mas->max) &&
        !mas_slot_locked(r_mas, r_wr_mas->slots, r_mas->offset + 1)) {
        r_mas->last =
            mas_safe_pivot(r_mas, r_wr_mas->pivots, r_wr_mas->type, static_cast<maple_type>(r_mas->offset + 1));
        r_mas->offset++;
    }
}
...
static inline int mas_new_root(struct ma_state *mas, void *entry) {
    struct maple_enode *root = static_cast<maple_enode *>(mas_root_locked(mas));
    enum maple_type type = maple_leaf_64;
    struct maple_node *node;
    void __rcu **slots;
    unsigned long *pivots;
...
static inline int mas_wr_spanning_store(struct ma_wr_state *wr_mas) {
    ...
    if (r_mas.offset <= r_wr_mas.node_end)
        mas_mab_cp(&r_mas, r_mas.offset, r_wr_mas.node_end, &b_node, b_node.b_end + 1);
    else
        b_node.b_end++;
 ...
 */
static inline bool mas_wr_node_store(struct ma_wr_state *wr_mas, unsigned char new_end) {
    struct ma_state *mas = wr_mas->mas;
    ...
    if (!mte_is_root(mas->node) && (new_end <= mt_min_slots[wr_mas->type]) && !(mas->mas_flags & MA_STATE_BULK))
        return false;
    ...
    memcpy(dst_pivots + dst_offset, wr_mas->pivots + offset_end, sizeof(unsigned long) * (copy_size - 1));
...
static inline void mas_wr_extend_null(struct ma_wr_state *wr_mas) {
    ...
    if (!wr_mas->slots[wr_mas->offset_end]) {
        ...
        if ((mas->last == wr_mas->end_piv) && (wr_mas->node_end != wr_mas->offset_end) &&
    ...
    if (!wr_mas->content) {
        ...
        if (mas->index == wr_mas->r_min && mas->offset && !wr_mas->slots[mas->offset - 1]) {
            mas->offset--;
            wr_mas->r_min = mas->index = mas_safe_min(mas, wr_mas->pivots, mas->offset);
...
static inline void mas_wr_end_piv(struct ma_wr_state *wr_mas) {
    while ((wr_mas->offset_end < wr_mas->node_end) && (wr_mas->mas->last > wr_mas->pivots[wr_mas->offset_end]))
        wr_mas->offset_end++;
 ...
 */
static inline bool mas_wr_append(struct ma_wr_state *wr_mas, unsigned char new_end) {
    struct ma_state *mas;
    void __rcu **slots;
    unsigned char end;
...
static inline bool mas_rewalk_if_dead(struct ma_state *mas, struct maple_node *node, const unsigned long index) {
    if (unlikely(ma_dead_node(node))) {
...
static inline int mas_prev_node(struct ma_state *mas, unsigned long min) {
    ...
    while (level > 1) {
        level--;
        slots = ma_slots(node, mt);
        mas->node = static_cast<maple_enode *>(mas_slot(mas, slots, offset));
        if (unlikely(ma_dead_node(node)))
            return 1;
 ...
 */
static void *mas_prev_slot(struct ma_state *mas, unsigned long min, bool empty, bool set_underflow) {
    void *entry;
    void __rcu **slots;
    unsigned long pivot;
    enum maple_type type;
 ...
 * Return: 1 on dead node, 0 otherwise.
 */
static inline int mas_next_node(struct ma_state *mas, struct maple_node *node, unsigned long max) {
    unsigned long min;
    unsigned long *pivots;
    struct maple_enode *enode;
    int level = 0;
    ...
    slots = ma_slots(node, mt);
    mas->offset++;
    enode = static_cast<maple_enode *>(mas_slot(mas, slots, mas->offset));
    if (unlikely(ma_dead_node(node)))
        return 1;
    ...
    while (unlikely(level > 1)) {
        ...
        mt = mte_node_type(mas->node);
        slots = ma_slots(node, mt);
        enode = static_cast<maple_enode *>(mas_slot(mas, slots, 0));
        if (unlikely(ma_dead_node(node)))
            return 1;
    }
 ...
 */
static void *mas_next_slot(struct ma_state *mas, unsigned long max, bool empty, bool set_overflow) {
    void __rcu **slots;
    unsigned long *pivots;
    unsigned long pivot;
    enum maple_type type;
 ...
 *
 */
static bool mas_rev_awalk(struct ma_state *mas, unsigned long size, unsigned long *gap_min, unsigned long *gap_max) {
    enum maple_type type = mte_node_type(mas->node);
    struct maple_node *node = mas_mn(mas);
    unsigned long *pivots, *gaps;
    void __rcu **slots;
...
static inline bool mas_anode_descend(struct ma_state *mas, unsigned long size) {
    ...
    for (; offset <= data_end; offset++) {
        ...
        if (gaps)
            ...
            gap = std::min(pivot, mas->last) - std::max(mas->index, min) + 1;
        else
            goto next_slot;
 ...
 */
static inline int mas_sparse_area(
    struct ma_state *mas,
    unsigned long min,
    unsigned long max,
    unsigned long size,
 ...
 */
static inline unsigned char mte_dead_leaves(struct maple_enode *enode, struct maple_tree *mt, void __rcu **slots) {
    struct maple_node *node;
    enum maple_type type;
    maple_enode *entry;
    int offset;
    ...
    for (offset = 0; offset < mt_slot_count(enode); offset++) {
        entry = static_cast<maple_enode *>(mt_slot(mt, slots, offset));
...
static void __rcu **mte_dead_walk(struct maple_enode **enode, unsigned char offset) {
    ...
    do {
        *enode = ma_enode_ptr(next);
        ...
        next = static_cast<maple_node *>(rcu_dereference(slots[offset]));
        offset = 0;
...
static void mt_free_walk(struct rcu_head *head) {
    ...
    node = caa_container_of(head, struct maple_node, rcu);
...
static inline void __rcu **mte_destroy_descend(
    struct maple_enode **enode,
    ...
    do {
        *enode = next;
        ...
        type = mte_node_type(*enode);
        slots = ma_slots(node, type);
        next = static_cast<maple_enode *>(mt_slot_locked(mt, slots, next_offset));
...
static void mt_destroy_walk(struct maple_enode *enode, struct maple_tree *mt, bool free) {
    ...
    do {
        ...
        tmp = static_cast<maple_enode *>(mt_slot_locked(mt, slots, offset));
 ...
 * Must hold the write lock.
 */
static inline void mte_destroy_walk(struct maple_enode *enode, struct maple_tree *mt) {
    struct maple_node *node = mte_to_node(enode);
...
static void mas_wr_store_setup(struct ma_wr_state *wr_mas) {
    ...
    if (mte_is_leaf(wr_mas->mas->node) && wr_mas->mas->last == wr_mas->mas->max)
        goto reset;
...
int mas_expected_entries(struct ma_state *mas, unsigned long nr_entries) {
     ...
     */
    nr_nodes = std::max(nr_entries, nr_entries * 2 + 1);
    if (!mt_is_alloc(mas->tree))
    ...
    mas_node_count_gfp(mas, nr_nodes + 3, /*GFP_KERNEL*/ 0);
...
static inline bool mas_next_setup(struct ma_state *mas, unsigned long max, void **entry) {
...
static inline bool mas_prev_setup(struct ma_state *mas, unsigned long min, void **entry) {
 ...
 */
static inline bool mas_find_setup(struct ma_state *mas, unsigned long max, void **entry) {
    if (mas_is_active(mas)) {
        if (mas->last < max)
            return false;
 ...
 * Returns: True if entry is the answer, false otherwise.
 */
static inline bool mas_find_rev_setup(struct ma_state *mas, unsigned long min, void **entry) {
    if (mas_is_active(mas)) {
        if (mas->index > min)
            return false;
 ...
 */
bool mas_nomem(struct ma_state *mas, gfp_t gfp) __must_hold(mas->tree->ma_lock) {
...
void __mt_destroy(struct maple_tree *mt) {
    ...
    if (xa_is_node(root))
        mte_destroy_walk(static_cast<maple_enode *>(root), mt);
 ...
 */
static inline struct maple_enode *mas_get_slot(struct ma_state *mas, unsigned char offset) {
    return mas_slot(mas, ma_slots(mas_mn(mas), mte_node_type(mas->node)), offset);
}
...
static void mt_dump_range(unsigned long min, unsigned long max, unsigned int depth, enum mt_dump_format format) {
...
static void mt_dump_range64(
    ...
    for (i = 0; i < MAPLE_RANGE64_SLOTS; i++) {
        ...
        if (last == 0 && i > 0)
            break;
        if (leaf)
            mt_dump_entry(mt_slot(mt, node->slot, i), first, last, depth + 1, format);
        else if (node->slot[i])
            mt_dump_node(mt, mt_slot(mt, node->slot, i), first, last, depth + 1, format);
...
static void mt_dump_node(
    ...
    pr_cont("node %p depth %d type %d parent %p", node, depth, type, node ? node->parent : NULL);
    switch (type) {
    case maple_dense:
        pr_cont("\n");
        for (i = 0; i < MAPLE_NODE_SLOTS; i++) {
            ...
            mt_dump_entry(mt_slot(mt, node->slot, i), min + i, min + i, depth, format);
        }
        break;
    case maple_leaf_64:
    case maple_range_64:
    ...
    for (i = 0; i < mt_slot_count(mte); i++) {
        ...
        if (!gaps) {
            ...
            if (gap > p_end - p_start + 1) {
                pr_err("%p[%u] %lu >= %lu - %lu + 1 (%lu)\n", mas_mn(mas), i, gap, p_end, p_start, p_end - p_start + 1);
...
static void mas_validate_child_slot(struct ma_state *mas) {
    ...
    for (i = 0; i < mt_slots[type]; i++) {
        ...
        if (mte_parent_slot(child) != i) {
            pr_err(
                "Slot error at %p[%u]: child %p has pslot %u\n",
                mas_mn(mas),
                i,
                mte_to_node(child),
        ...
        if (mte_parent(child) != mte_to_node(mas->node)) {
            pr_err("child %p has parent %p not %p\n", mte_to_node(child), mte_parent(child), mte_to_node(mas->node));
            MT_BUG_ON(mas->tree, 1);
        }
...
static void mas_validate_limits(struct ma_state *mas) {
    ...
    for (i = 0; i < mt_slots[type]; i++) {
        ...
        if (!piv && (i != 0)) {
            pr_err("Missing node limit pivot at %p[%u]", mas_mn(mas), i);
            MAS_WARN_ON(mas, 1);
        }
...
void mt_validate(struct maple_tree *mt) {
    ...
    while (!mas_is_none(&mas)) {
        ...
        if (MAS_WARN_ON(&mas, (end < mt_min_slot_count(mas.node)) && (mas.max != ULONG_MAX))) {
...
void mas_dump(const struct ma_state *mas) {
    ...
    pr_err(
        "     min=%lx max=%lx alloc=%p, depth=%u, flags=%x\n",
        mas->min,
        mas->max,
        mas->alloc,
        mas->depth,
...
void mas_wr_dump(const struct ma_wr_state *wr_mas) {
    ...
    pr_err(
        "        type=%u off_end=%u, node_end=%u, end_piv=%lx\n",
        wr_mas->type,
        wr_mas->offset_end,
        wr_mas->node_end,
        wr_mas->end_piv);
