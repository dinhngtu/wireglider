...
struct NetPrefix4 {
    ...
    constexpr NetPrefix4(in_addr, unsigned int) {
    }
    explicit constexpr NetPrefix4(const IpRange4 &net) {
    }
...
struct NetPrefix6 {
    static const unsigned int bits = CHAR_BIT * sizeof(unsigned long);
    constexpr NetPrefix6() {
    }
    constexpr NetPrefix6(in6_addr net, unsigned int prefix) : quantum(std::max(prefix, bits) - bits) {
    }
