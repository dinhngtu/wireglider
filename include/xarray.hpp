...
static inline void xa_init_flags(struct xarray *xa, gfp_t flags) {
    pthread_mutex_init(&xa->xa_lock, NULL);
    xa->xa_flags = flags;
    xa->xa_head = NULL;
}
...
#define xa_trylock(xa) pthread_mutex_trylock(&(xa)->xa_lock)
#define xa_lock(xa) pthread_mutex_lock(&(xa)->xa_lock)
#define xa_unlock(xa) pthread_mutex_unlock(&(xa)->xa_lock)
#define xa_lock_bh(xa) pthread_mutex_lock(&(xa)->xa_lock)
#define xa_unlock_bh(xa) pthread_mutex_unlock(&(xa)->xa_lock)
#define xa_lock_irq(xa) pthread_mutex_lock(&(xa)->xa_lock)
#define xa_unlock_irq(xa) pthread_mutex_unlock(&(xa)->xa_lock)
#define xa_lock_irqsave(xa, flags) pthread_mutex_lock(&(xa)->xa_lock)
#define xa_unlock_irqrestore(xa, flags) pthread_mutex_unlock(&(xa)->xa_lock)
/*
...
int __must_check __xa_alloc(struct xarray *, uint32_t *id, void *entry, struct xa_limit, gfp_t);
int __must_check __xa_alloc_cyclic(struct xarray *, uint32_t *id, void *entry, struct xa_limit, uint32_t *next, gfp_t);
 ...
 */
static inline __must_check int xa_alloc(
    struct xarray *xa,
    uint32_t *id,
    void *entry,
...
xa_alloc_bh(struct xarray *xa, uint32_t *id, void *entry, struct xa_limit limit, gfp_t gfp) {
    int err;
 ...
 */
static inline int __must_check
xa_alloc_irq(struct xarray *xa, uint32_t *id, void *entry, struct xa_limit limit, gfp_t gfp) {
    int err;
 ...
 * allocated or -EBUSY if there are no free entries in @limit.
 */
static inline int xa_alloc_cyclic_bh(
    struct xarray *xa,
    uint32_t *id,
    void *entry,
    struct xa_limit limit,
 ...
 * allocated or -EBUSY if there are no free entries in @limit.
 */
static inline int xa_alloc_cyclic_irq(
    struct xarray *xa,
    uint32_t *id,
    void *entry,
    struct xa_limit limit,
...
static inline bool xa_is_sibling(const void *entry) {
    return /*IS_ENABLED(CONFIG_XARRAY_MULTI)*/ false && xa_is_internal(entry) &&
...
#define __XA_STATE(array, index, shift, sibs)                                                            \
    {                                                                                                    \
        .xa = array, .xa_index = index, .xa_shift = shift, .xa_sibs = sibs, .xa_offset = 0, .xa_pad = 0, \
        .xa_node = XAS_RESTART, .xa_alloc = NULL, .xa_update = NULL, .xa_lru = NULL,                     \
    }
...
#define XA_STATE_ORDER(name, array, index, order) \
    struct xa_state name = __XA_STATE(            \
        array,                                    \
        (index >> order) << order,                \
...
static inline void *xas_reload(struct xa_state *xas) {
    ...
    if (/*IS_ENABLED(CONFIG_XARRAY_MULTI)*/ false) {
