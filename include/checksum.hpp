...
static inline uint64_t checksum_nofold(const std::span<uint8_t, 16> b, uint64_t initial) {
    uint64_t carry = 0;
...
static inline uint64_t checksum_nofold(const std::span<uint8_t, 8> b, uint64_t initial) {
    uint64_t ret;
    bool c = __builtin_add_overflow(initial, *reinterpret_cast<uint64_t *>(&b[0]), &ret);
    return ret + c;
}

static inline uint64_t checksum_nofold(const std::span<uint8_t, 4> b, uint64_t initial) {
    uint64_t ret;
    bool c = __builtin_add_overflow(initial, static_cast<uint64_t>(*reinterpret_cast<uint32_t *>(&b[0])), &ret);
    return ret + c;
}

static inline uint64_t checksum_nofold(const std::span<uint8_t, 2> b, uint64_t initial) {
    uint64_t ret;
    bool c = __builtin_add_overflow(initial, static_cast<uint64_t>(*reinterpret_cast<uint16_t *>(&b[0])), &ret);
    return ret + c;
}

static inline uint64_t checksum_nofold(const std::span<uint8_t, 1> b, uint64_t initial) {
    uint64_t carry = 0;
...
#ifdef __x86_64__
static inline uint64_t checksum_nofold(const std::span<uint8_t, std::dynamic_extent> b, uint64_t initial) {
...
static inline uint64_t pseudo_header_checksum_nofold(
    uint8_t proto,
    const std::span<uint8_t, E1> srcAddr,
    const std::span<uint8_t, E2> dstAddr,
    uint16_t totalLen) {
...
static inline uint16_t checksum(const std::span<uint8_t> b, uint64_t initial) {
